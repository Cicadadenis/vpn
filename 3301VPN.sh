...
read YOUR_PASSWORD
echo ""
echo "Какой порт вы хотите, чтобы CicadaVPN установил ?"
...
2)
  until [[ $PORT =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; do
    read -rp "Выбор порта [1-65535]: " -e -i 1194 PORT
  done
  ;;
3)
...
Сервер IPsec VPN теперь готов к использованию!
...
Запишите это. Они понадобятся вам для подключения!
