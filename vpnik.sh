...
Options:
  --auto                        запустите настройку IKEv2 в автоматическом режиме с параметрами по умолчанию (только для начальной настройки IKEv2)
...
check_arguments() {
  ...
  if [ "$remove_ikev2" = "1" ]; then
    ...
    if [ "$((add_client_using_defaults + export_client_using_defaults + list_clients + use_defaults))" -gt 0 ]; then
      show_usage "Invalid parameters. '--removeikev2' нельзя указать с другими параметрами."
    fi
  fi
}
...
check_server_dns_name() {
  if [ -n "$VPN_DNS_NAME" ]; then
    check_dns_name "$VPN_DNS_NAME" || exiterr "Invalid DNS name. 'VPN_DNS_NAME' должно быть полное доменное имя (FQDN)."
  fi
}
...
check_custom_dns() {
  if { [ -n "$VPN_DNS_SRV1" ] && ! check_ip "$VPN_DNS_SRV1"; } \
    ...
    exiterr "The DNS указанный сервер недействителен."
  fi
}
...
run_swan_update() {
  ...
  if [ -d "$TMPDIR" ]; then
    ...
    { set +x; } 2>&-
    [ ! -s "$TMPDIR/vpnupg.sh" ] && echo "Error: Не удалось загрузить скрипт обновления." >&2
    /bin/rm -f "$TMPDIR/vpnupg.sh"
    /bin/rmdir "$TMPDIR"
  else
    echo "Error: Не удалось создать временный каталог." >&2
  fi
...
select_swan_update() {
  if printf '%s' "$swan_ver_latest" | grep -Eq '^([3-9]|[1-9][0-9])\.([0-9]|[1-9][0-9])$' \
    ...
    echo "Note: Более новая версия VPN ($swan_ver_latest) is available."
    ...
    if [ "$in_container" = "0" ]; then
      ...
      echo "      Чтобы обновить этот образ Docker, see: https://git.io/updatedockervpn"
      echo
      ...
      case $response in
        [yY][eE][sS]|[yY])
          ...
          ;;
        *)
          echo "Abort. Никаких изменений не было."
          exit 1
          ;;
      esac
    fi
...
cat <<'EOF'
Добро пожаловать! Используйте этот сценарий для настройки IKEv2 после настройки собственной службы IPsec VPN.r.
Кроме того, вы можете вручную настроить IKEv2. Видеть: https://git.io/ikev2

Прежде чем приступить к настройке, мне нужно задать вам несколько вопросов.
Вы можете использовать параметры по умолчанию и просто нажать клавишу ВВОД, если они вас устраивают.
...
enter_server_address() {
  echo "Вы хотите, чтобы клиенты IKEv2 VPN подключались к этому серверу, используя DNS-имя, "
...
enter_client_name() {
  echo
  echo "Укажите имя для клиента IKEv2 VPN.."
  echo "Используйте только одно слово, никаких специальных символов, кроме '-' and '_'."
...
enter_client_name_with_defaults() {
  echo
  echo "Укажите имя для клиента IKEv2 VPN.."
  echo "Используйте только одно слово, никаких специальных символов, кроме '-' И '_'."
...
enter_custom_dns() {
  ...
  if [ "$use_custom_dns" = "1" ]; then
    ...
    until check_ip "$dns_server_1"; do
      ...
      read -rp "Недействительный DNS-сервер Введите основной DNS-сервер: " dns_server_1
    done
...
EOF
  ...
  if [ "$client_validity" = "1" ]; then
    ...
    echo "Сертификат клиента действителен в течение: $ client_validity месяцев"
  fi
