...
Options:
  ...
  --listclients                 list the names of existing IKEv2 clients (after cicada setup)
  --removeikev2                 remove cicada and delete all certificates and keys from the IPsec database
...
check_arguments() {
  ...
  if [ "$add_client_using_defaults" = "1" ]; then
    ! check_ikev2_exists && exiterr "You must first set up cicada before adding a new client."
  ...
  if [ "$export_client_using_defaults" = "1" ]; then
    ! check_ikev2_exists && exiterr "You must first set up cicada before exporting a client configuration."
    get_server_address
