...
toilet -f smblock --filter border:metal:flip:flop:flip:flop 'Cicada-IKEv2-VPN'
...
Параметры:
  --auto                        запустите настройку Cicada в автоматическом режиме с параметрами по умолчанию (только для начальной настройки Cicada)
  --addclient [client name]     добавить нового клиента Cicada с параметрами по умолчанию (после установки Cicada)
  --exportclient [client name]  экспортировать существующий клиент Cicada, используя параметры по умолчанию (после настройки Cicada)
  --listclients                 перечислить имена существующих клиентов Cicada (после настройки Cicada)
  --removeden2                  удалить Cicada и удалить все сертификаты и ключи из базы данных IPsec
  -h, --help                    показать это справочное сообщение и выйти

Чтобы настроить Cicada или параметры клиента, запустите этот сценарий без аргументов..
EOF
  exit 1
}

check_den2_exists() {
  grep -qs "conn den2-cp" /etc/ipsec.conf || [ -f /etc/ipsec.d/den2.conf ]
}
...
check_arguments() {
  ...
  if [ "$add_client_using_defaults" = "1" ]; then
    ! check_den2_exists && exiterr "Перед добавлением нового клиента необходимо сначала настроить Cicada.."
  ...
  if [ "$export_client_using_defaults" = "1" ]; then
    ! check_den2_exists && exiterr "Перед экспортом конфигурации клиента необходимо сначала настроить Cicada.."
    get_server_address
  ...
  if [ "$list_clients" = "1" ]; then
    ! check_den2_exists && exiterr "Перед перечислением клиентов необходимо сначала настроить Cicada.."
  fi
  if [ "$remove_den2" = "1" ]; then
    ! check_den2_exists && exiterr "Невозможно удалить Cicada, потому что он не был настроен на этом сервере."
    if [ "$((add_client_using_defaults + export_client_using_defaults + list_clients + use_defaults))" -gt 0 ]; then
      show_usage "Неверные параметры. '--removeden2' нельзя указать с другими параметрами."
    fi
  fi
}
...
check_ca_cert_exists() {
  if certutil -L -d sql:/etc/ipsec.d -n "Cicada VPN CA" >/dev/null 2>&1; then
    exiterr "Сертификат 'Cicada VPN CA' уже существует."
  fi
}
...
check_swan_ver() {
  if [ "$in_container" = "0" ]; then
    swan_ver_url="https://dl.ls20.com/v1/$os_type/$os_ver/swanver?arch=$os_arch&ver=$swan_ver&auto=$use_defaults"
  else
    swan_ver_url="https://dl.ls20.com/v1/docker/$os_arch/swanverikev22?ver=$swan_ver&auto=$use_defaults"
  fi
...
run_swan_update() {
  ...
  read -n 1 -s -r -p "Нажмите любую клавишу, чтобы продолжить настройку Cicada...."
  echo
}
...
select_swan_update() {
  if printf '%s' "$swan_ver_latest" | grep -Eq '^([3-9]|[1-9][0-9])\.([0-9]|[1-9][0-9])$' \
    ...
    echo "      Перед настройкой Cicada рекомендуется обновить Libreswan.."
    if [ "$in_container" = "0" ]; then
      echo
...
toilet -f smblock --filter   border:metal:flip:flop:flip:flop 'Cicada-VPN'

show_welcome_message() {
cat <<'EOF'
Добро пожаловать! Используйте этот сценарий для настройки Cicada после настройки собственного сервера IPsec VPN.
Кроме того, вы можете вручную настроить Cicada. Смотреть тут >>> : https://git.io/ikev2
...
show_start_message() {
  bigecho "Запуск установки Cicada в автоматическом режиме с параметрами по умолчанию."
}

show_add_client_message() {
  bigecho "Добавление нового клиента Cicada '$client_name', с использованием параметров по умолчанию."
}

show_export_client_message() {
  bigecho "Экспорт существующего клиента Cicada '$client_name', с использованием параметров по умолчанию."
}
...
get_server_address() {
  server_addr=$(grep -s "leftcert=" /etc/ipsec.d/den.conf | cut -f2 -d=)
...
list_existing_clients() {
  echo "Проверка существующих клиентов Cicada..."
  certutil -L -d sql:/etc/ipsec.d | grep -v -e '^$' -e 'Cicada VPN CA' -e '\.' | tail -n +3 | cut -f1 -d ' '
}

enter_server_address() {
  echo "Вы хотите, чтобы клиенты Cicada VPN подключались к этому серверу с использованием DNS-имени?,"
...
enter_client_name() {
  echo
  echo "Укажите имя для VPN-клиента Cicada.."
...
enter_client_name_with_defaults() {
  ...
  echo "Укажите имя для VPN-клиента Cicada.."
  echo "Используйте только одно слово, никаких специальных символов, кроме '-' а также '_'."
  read -rp "Имя клиента: [Cicada]" client_name
...
enter_client_name_for_export() {
  ...
  get_server_address
  echo
  read -rp "Введите имя клиента Cicada для экспорта: " client_name
...
select_mobike() {
  ...
  if [ "$mobike_support" = "1" ]; then
    echo
    echo "Расширение MOBIKE  позволяет клиентам VPN изменять точки подключения к сети,"
...
select_menu_option() {
  echo "Cicada уже настроен на этом сервере."
  echo
...
cat <<EOF
Теперь мы готовы к настройке Cicada. Ниже приведены выбранные вами параметры настройки.
...
create_mobileconfig() {
  ...
  [ -z "$ca_base64" ] && exiterr "Не удалось закодировать сертификат CA Cicada VPN."
...
add_den2_connection() {
  bigecho2 "Добавление нового соединения Cicada..."
...
EOF
  ...
  if [ "$mobike_enable" = "1" ]; then
    echo "  mobike=yes" >> /etc/ipsec.d/den2.conf
  else
    echo "  mobike=no" >> /etc/ipsec.d/den2.conf
  fi
}
...
Cicada VPN-клиент "$client_name" экспортируется!
...
Установка Cicada прошла успешно. Подробная информация о режиме Cicada:
...
Следующие шаги: Настройка клиентов Cicada VPN. см.:
...
check_ipsec_conf() {
  if grep -qs "conn den2-cp" /etc/ipsec.conf; then
    echo "Ошибка: раздел конфигурации Cicada найден в /etc/ipsec.conf." >&2
    echo "       Этот сценарий не может автоматически удалить Cicada с этого сервера.." >&2
    echo "       Чтобы вручную удалить Cicada, см. https://git.io/ikev2" >&2
    echo "Прервать. Никаких изменений не было." >&2
    exit 1
  fi
}
...
confirm_remove_den2() {
  echo
  echo "ВНИМАНИЕ: эта опция удалит Cicada с этого VPN-сервера, но сохранит IPsec / L2TP."
  echo "         а также IPsec/XAuth (\"Cisco IPsec\") режимы, если установлены Все Cicada конфигурации"
  echo "         including certificates and keys will be permanently deleted."
  echo "         Это не может быть отменено! "
  echo
  printf "Вы уверены, что хотите удалить Cicada? [y/N] "
  read -r response
  case $response in
    [yY][eE][sS]|[yY])
      echo
...
delete_den2_conf() {
  bigecho "Deleting /etc/ipsec.d/den2.conf..."
  /bin/rm -f /etc/ipsec.d/den2.conf
}
