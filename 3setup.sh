...
Options:
  ...
  --addclient [client name]     добавить нового клиента IKEv2, используя параметры по умолчанию (после настройки IKEv2)
  --exportclient [client name]  экспортировать существующий клиент IKEv2 с использованием параметров по умолчанию (после настройки IKEv2)
  --listclients                 перечислить имена существующих клиентов IKEv2 (после настройки IKEv2)
  --removeikev2                 удалить IKEv2 и удалить все сертификаты и ключи из базы данных IPsec
  -h, --help                    sкак это справочное сообщение и выход

Чтобы настроить IKEv2 или параметры клиента, запустите этот сценарий без аргументов..
EOF
  exit 1
}
...
check_arguments() {
  ...
  if [ "$add_client_using_defaults" = "1" ]; then
    ! check_ikev2_exists && exiterr "Перед добавлением нового клиента необходимо сначала настроить IKEv2.."
  ...
  if [ "$export_client_using_defaults" = "1" ]; then
    ! check_ikev2_exists && exiterr "Перед экспортом конфигурации клиента необходимо сначала настроить IKEv2.."
    get_server_address
  ...
  if [ "$remove_ikev2" = "1" ]; then
    ! check_ikev2_exists && exiterr "Невозможно удалить IKEv2, потому что он не был настроен на этом сервере."
...
run_swan_update() {
  ...
  read -n 1 -s -r -p "Нажмите любую клавишу, чтобы продолжить настройку IKEv2...."
  echo
}
...
select_swan_update() {
  if printf '%s' "$swan_ver_latest" | grep -Eq '^([3-9]|[1-9][0-9])\.([0-9]|[1-9][0-9])$' \
    ...
    echo "      Перед настройкой IKEv2 рекомендуется обновить Libreswan.."
    if [ "$in_container" = "0" ]; then
      echo
...
cat <<'EOF'
Добро пожаловать! Используйте этот сценарий для настройки IKEv2 после настройки собственного сервера IPsec VPN.
Кроме того, вы можете вручную настроить IKEv2. См .: https://git.io/ikev2
...
show_start_message() {
  bigecho "Запуск настройки IKEv2 в автоматическом режиме с параметрами по умолчанию."
}

show_add_client_message() {
  bigecho "Добавление нового клиента IKEv2 '$client_name', с использованием параметров по умолчанию."
}

show_export_client_message() {
  bigecho "Экспорт существующего клиента IKEv2 '$client_name', с использованием параметров по умолчанию."
}
...
list_existing_clients() {
  echo "Проверка существующих клиентов IKEv2..."
  certutil -L -d sql:/etc/ipsec.d | grep -v -e '^$' -e 'Cicada3301 CA' -e '\.' | tail -n +3 | cut -f1 -d ' '
}
...
enter_client_name_for_export() {
  ...
  get_server_address
  echo
  read -rp "Введите имя клиента IKEv2 для экспорта: " client_name
  while [ -z "$client_name" ] || ! check_client_name \
    || [ "$client_name" = "Cicada3301 CA" ] || [ "$client_name" = "$server_addr" ] \
...
enter_custom_dns() {
  ...
  printf "Вы хотите указать собственные DNS-серверы для IKEv2?? [y/N] "
  read -r response
  case $response in
    [yY][eE][sS]|[yY])
      use_custom_dns=1
...
select_menu_option() {
  echo "IKEv2 уже настроен на этом сервере."
  echo
...
cat <<EOF
Теперь мы готовы к настройке IKEv2. Ниже приведены выбранные вами параметры настройки.
...
create_client_cert() {
  ...
  certutil -z <(head -c 1024 /dev/urandom) \
    -S -c "Cicada3301 CA" -n "$client_name" \
    -s "O=Cicada3301,CN=$client_name" \
...
create_mobileconfig() {
  ...
  ca_base64=$(certutil -L -d sql:/etc/ipsec.d -n "Cicada3301 CA" -a | grep -v CERTIFICATE)
  [ -z "$ca_base64" ] && exiterr "Не удалось закодировать сертификат Cicada3301 CA."
...
$ca_base64
  ...
  <key>PayloadDisplayName</key>
  <string>Cicada3301 ($server_addr)</string>
  <key>PayloadIdentifier</key>
  <string>com.apple.vpn.managed.$(uuidgen)</string>
...
create_ca_server_certs() {
  ...
  certutil -z <(head -c 1024 /dev/urandom) \
    -S -x -n "Cicada3301 CA" \
    -s "O=Cicada3301,CN=Cicada3301 CA" \
    -k rsa -v 120 \
...
ANSWERS
  ...
  if [ "$use_dns_name" = "1" ]; then
    certutil -z <(head -c 1024 /dev/urandom) \
      -S -c "Cicada3301 CA" -n "$server_addr" \
      -s "O=Cicada3301,CN=$server_addr" \
      -k rsa -v 120 \
      -d sql:/etc/ipsec.d -t ",," \
      --keyUsage digitalSignature,keyEncipherment \
...
add_ikev2_connection() {
  bigecho2 "Добавление нового соединения IKEv2..."
...
Новый VPN-клиент IKEv2 "$client_name" добавлен!
...
Настройка IKEv2 прошла успешно. Подробная информация о режиме IKEv2:
...
check_ipsec_conf() {
  if grep -qs "conn ikev2-cp" /etc/ipsec.conf; then
    ...
    echo "       Этот сценарий не может автоматически удалить IKEv2 с этого сервера.." >&2
    echo "       Чтобы вручную удалить IKEv2, см. https://git.io/ikev2" >&2
    echo "Прервать. Никаких изменений не было." >&2
    exit 1
  fi
}
...
confirm_remove_ikev2() {
  echo
  echo "ВНИМАНИЕ! Эта опция удалит IKEv2 с этого VPN-сервера, но сохранит IPsec / L2TP."
  echo "         а также IPsec/XAuth (\"Cisco IPsec\")режимы, если установлены. Вся конфигурация IKEv2"
  echo "         включая сертификаты и ключи будут безвозвратно удалены."
  echo "         Это не может быть отменено! "
  echo
  printf "Вы уверены, что хотите удалить IKEv2? [y/N] "
  read -r response
  case $response in
    [yY][eE][sS]|[yY])
      echo
...
delete_certificates() {
  ...
  certutil -L -d sql:/etc/ipsec.d | grep -v -e '^$' -e 'Cicada3301 CA' | tail -n +3 | cut -f1 -d ' ' | while read -r line; do
    certutil -F -d sql:/etc/ipsec.d -n "$line"
    certutil -D -d sql:/etc/ipsec.d -n "$line" 2>/dev/null
  done
  certutil -F -d sql:/etc/ipsec.d -n "Cicada3301 CA"
  certutil -D -d sql:/etc/ipsec.d -n "Cicada3301 CA" 2>/dev/null
}
