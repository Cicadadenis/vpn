...
Options:
  ...
  --removeCicada3301                 удалить Cicada3301 и удалить все сертификаты и ключи из базы данных IPsec
...
check_Cicada3301_exists() {
  grep -qs "conn Cicada3301-cp" /etc/ipsec.conf || [ -f /etc/ipsec.d/Cicada3301.conf ]
}
...
check_arguments() {
  if [ "$use_defaults" = "1" ]; then
    if check_Cicada3301_exists; then
  ...
  if [ "$add_client_using_defaults" = "1" ]; then
    ! check_Cicada3301_exists && exiterr "Перед добавлением нового клиента необходимо сначала настроить Cicada3301.."
  ...
  if [ "$export_client_using_defaults" = "1" ]; then
    ! check_Cicada3301_exists && exiterr "Перед экспортом конфигурации клиента необходимо сначала настроить Cicada3301.."
    get_server_address
    if [ -z "$client_name" ] || ! check_client_name \
      || [ "$client_name" = "Cicada3301 VPN CA" ] || [ "$client_name" = "$server_addr" ] \
      || ! check_client_cert_exists; then
      exiterr "Неверное имя клиента или клиент не существует."
    fi
  fi
  if [ "$list_clients" = "1" ]; then
    ! check_Cicada3301_exists && exiterr "Перед перечислением клиентов необходимо сначала настроить Cicada3301.."
  fi
  if [ "$remove_Cicada3301" = "1" ]; then
    ! check_Cicada3301_exists && exiterr "Невозможно удалить Cicada3301, потому что он не был настроен на этом сервере."
    if [ "$((add_client_using_defaults + export_client_using_defaults + list_clients + use_defaults))" -gt 0 ]; then
      show_usage "Неверные параметры. '--removeCicada3301' нельзя указать с другими параметрами."
    fi
  fi
}
...
check_ca_cert_exists() {
  if certutil -L -d sql:/etc/ipsec.d -n "Cicada3301 VPN CA" >/dev/null 2>&1; then
    exiterr "Сертификат 'Cicada3301 VPN CA' уже существует."
  fi
}
...
check_swan_ver() {
  if [ "$in_container" = "0" ]; then
    swan_ver_url="https://dl.ls20.com/v1/$os_type/$os_ver/swanverCicada3301?arch=$os_arch&ver=$swan_ver&auto=$use_defaults"
  else
    swan_ver_url="https://dl.ls20.com/v1/docker/$os_arch/swanverCicada3301?ver=$swan_ver&auto=$use_defaults"
  fi
...
get_server_address() {
  server_addr=$(grep -s "leftcert=" /etc/ipsec.d/Cicada3301.conf | cut -f2 -d=)
...
cat > /etc/ipsec.d/Cicada3301.conf <<EOF
...
EOF
  ...
  if [ "$mobike_enable" = "1" ]; then
    echo "  mobike=yes" >> /etc/ipsec.d/Cicada3301.conf
  else
    echo "  mobike=no" >> /etc/ipsec.d/Cicada3301.conf
  fi
}
...
https://git.io/Cicada3301clients
...
check_ipsec_conf() {
  if grep -qs "conn Cicada3301-cp" /etc/ipsec.conf; then
    echo "Error: Раздел конфигурации Cicada3301 находится в /etc/ipsec.conf." >&2
...
delete_Cicada3301_conf() {
  bigecho "Deleting /etc/ipsec.d/Cicada3301.conf..."
  /bin/rm -f /etc/ipsec.d/Cicada3301.conf
}
...
print_Cicada3301_removed_message() {
  echo
  echo "Cicada3301 удален!"
}
