...
list_existing_clients() {
  ...
  certutil -L -d sql:/etc/ipsec.d | grep -v -e '^$' -e 'IKEv2 CA' -e '\.' | tail -n +3 | cut -f1 -d ' '
}
...
enter_client_name_for_export() {
  ...
  while [ -z "$client_name" ] || ! check_client_name \
    || [ "$client_name" = "IKEv2 CA" ] || [ "$client_name" = "$server_addr" ] \
...
create_client_cert() {
  ...
  certutil -z <(head -c 1024 /dev/urandom) \
    -S -c "IKEv2 CA" -n "$client_name" \
    -s "O=IKEv2,CN=$client_name" \
...
create_mobileconfig() {
  ...
  ca_base64=$(certutil -L -d sql:/etc/ipsec.d -n "IKEv2 CA" -a | grep -v CERTIFICATE)
...
delete_certificates() {
  ...
  certutil -L -d sql:/etc/ipsec.d | grep -v -e '^$' -e 'IKEv2 CA' | tail -n +3 | cut -f1 -d ' ' | while read -r line; do
    certutil -F -d sql:/etc/ipsec.d -n "$line"
    certutil -D -d sql:/etc/ipsec.d -n "$line" 2>/dev/null
  done
  certutil -F -d sql:/etc/ipsec.d -n "IKEv2 CA"
  certutil -D -d sql:/etc/ipsec.d -n "IKEv2 CA" 2>/dev/null
}
